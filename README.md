...
```mermaid
graph TD
    Device["M5Atom S3 Coaster"] -- "センサーデータ (5分周期/補給時即時)" --> Enebular["enebular クラウド実行環境"]
    Enebular -- "保存" --> Datastore["enebular データストア"]
...
* **応答トリガーキーワード**:
  `環境, 状態, 部屋, 温度, 室温, 湿度, 気圧, wbgt, 熱中症, 調子, 元気, ステータス, 天気, 暑い, 寒い, こまめ`
